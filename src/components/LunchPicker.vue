...
export default {
  data() {
    return {
      allTypes: ['飯', '麵', '水餃', '火鍋', '中式', '西式', '早餐', '線上支付', '遠', '巨無霸'],
      selectedTypes: [],
      picked: null,
      restaurants: [
        {
  ...
  methods: {
    pickRestaurant() {
      const filtered = this.selectedTypes.length
        ? this.restaurants.filter(r => r.types.some(t => this.selectedTypes.includes(t)))
        : this.restaurants;
      ...
      this.picked = filtered[randomIndex];
    }
  }
}
</script>
